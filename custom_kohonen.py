...
class KohonenMap(Module):
    """ Implements a Self-Organizing Map (SOM), also known as a Kohonen Map.
        ...
        neighbourhood relationship on a 2-dimensional (nNeurons x mNeurons)
        grid.

        dim - dimensionality of the input vector
        nNeurons, mNeurons - grid dimensions (5x3 grid has 15 neurons)
        
        There are two versions: With the outputFullMap option set to True, it
        outputs the full Kohonen map to the next layer, set to False it will
        only return 2 values: the x and y coordinate of the winner neuron. """
    ...
    def _backwardImplementation(self, outerr, inerr, outbuf, inbuf):
        ...
        br = (self.winner + n + 1)
        tl[tl < 0] = 0
        br[br > self.neighbourdist + 1] = self.neighbourdist + 1

        # calculate distance matrix
        tempm = 1 - sum(abs(self.distmatrix - self.winner.reshape(1, 1, 2)), 2) / self.neighbourdist

        tempm[tempm < 0] = 0
        distm = zeros((self.nNeurons, self.mNeurons, self.nInput))
